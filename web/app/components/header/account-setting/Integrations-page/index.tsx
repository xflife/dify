      ...
      <div className='mb-8'>
        <div className={titleClassName}>{t('common.integrations.connected')}</div>
        {
          integrates.map(integrate => (
            <div key={integrate.provider} className='mb-2 flex items-center px-3 py-2 bg-gray-50 border-[0.5px] border-[#F1F3F9] rounded-lg'>
              ...
              </div>
              {
                !integrate.is_bound && (
                  <Link
                    className='flex items-center h-8 px-[7px] bg-white rounded-lg border border-[#F1F3F9] text-xs font-medium text-gray-700 cursor-pointer'
                    href={integrate.link}
                    target={'_blank'}>
      ...
      {/* <div className='mb-8'>
        <div className={titleClassName}>Add a service </div>
        {
          services.map(service => (
            <div key={service.key} className='mb-2 flex items-center px-3 py-2 bg-gray-50 border-[0.5px] border-[#F1F3F9] rounded-lg'>
